...
describe('feed-slice', () => {
  ...
  describe('getFeeds', () => {
    ...
    it('должен обрабатывать состояние "rejected", когда запрос к API не удался', async () => {
      ...
      const feedsState = state.feeds;
      expect(feedsState.loading).toBe(false);
      expect(feedsState.error).toEqual(mockError);
  ...
  describe('getFeedById', () => {
    ...
    it('должен обрабатывать состояние отклонения, когда вызов API завершается с ошибкой', async () => {
      const errorMessage = 'Не удалось получить oder';
